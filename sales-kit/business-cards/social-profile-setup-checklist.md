...
- Facebook/TikTok public links must open without login before being added to the website as proof links. Facebook is updated in authenticated Page mode and needs a final logged-out cache-propagation check; TikTok is branded in the authenticated session but still redirects to mandatory login when checked logged out. Instagram may show a platform login wall, so it must not be used as the only verification proof.
