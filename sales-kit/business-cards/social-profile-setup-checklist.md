...
- Facebook Page exists at `https://www.facebook.com/profile.php?id=61589398630376` / `https://www.facebook.com/people/Cantoni-Digital-Studio/61589398630376/`. Meta blocked duplicate creation because the account already manages a Page named `Cantoni Digital Studio`. Browser entered Page management mode on 2026-05-12, disabled the promotional-email opt-in in the welcome modal, and confirmed the profile already shows the studio logo and `Web designer` category. Logged-out/public Browser QA shows page name, follower area, category and bio; contact cleanup is pending because re-login is blocked by Meta reCAPTCHA.
- Facebook cover asset is generated by `npm run build:social` as `sales-kit/social-launch/output/facebook-cover-cantoni.png` at 1640 x 624. Use it for the Page cover after reCAPTCHA is cleared.
...
- Facebook/TikTok public links must open without login before being added to the website as proof links. Facebook now passes a basic logged-out brand visibility check but still needs contact cleanup; TikTok is branded in the authenticated session but still redirects to mandatory login when checked logged out. Instagram may show a platform login wall, so it must not be used as the only verification proof.
