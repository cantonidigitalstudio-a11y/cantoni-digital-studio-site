...
- Historical note superseded by the 2026-05-14 QA below: Instagram authenticated Browser QA confirmed the profile existed with brand name, category, bio, 3 posts and visible follower data, but the clickable profile website link still used `zumu.be/ecantoni` and Instagram web blocked link edits. The required fix was completed on 2026-05-14, when the profile link was changed to `https://cantonidigitalstudio.com` and the stale link no longer appeared.
...
- iPad wireless fallback is now operational for future mobile-only flows: Developer Mode enabled, CoreDevice `transportType: localNetwork`, network discovery sees the iPad, and display inspection works after unplugging USB. Current app inventory now lists both Instagram and WebDriverAgent, but WebDriverAgent still needs iOS developer-certificate trust before automated control can run. This is no longer a blocker for the Instagram link/avatar task because the 2026-05-14 QA below confirms it is resolved.
