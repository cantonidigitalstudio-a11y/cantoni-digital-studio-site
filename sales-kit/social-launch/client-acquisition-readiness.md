...
- TikTok: account `@cantonidigitalstudio` creato, accesso completato tramite Google OAuth e profilo brandizzato con logo, nome e bio. QA autenticata del 2026-05-14 passata; QA logged-out rifatta lo stesso giorno: TikTok rimanda ancora a login obbligatorio, quindi non usarlo come prova autonoma. Evidenze: `/tmp/tiktok-profile-after-google.png` e `/tmp/tiktok-cantoni-public-google-final.png`.
- iPad fallback: collegamento wireless verificato il 2026-05-13 (`transportType: localNetwork`, `idevice_id -n` vede l'iPad, display inspection attiva). Instagram ora risulta installato sull'iPad. Il runner WebDriverAgent è stato firmato e installato, ma iOS richiede ancora il trust manuale del certificato sviluppatore in `Impostazioni > Generali > VPN e gestione dispositivo` prima di permettere future automazioni mobile-only. Il task Instagram link/avatar non dipende più da questo fallback perché il QA del 2026-05-14 lo conferma risolto.
...
7. Usare Facebook e YouTube come prove pubbliche; non usare TikTok come prova autonoma finché non passa QA pubblica senza login.
...
11. Mandare prima email senza prezzi e senza preventivo completo.
12. Preparare preventivo solo se il lead risponde o è top tier.
13. Tracciare ogni lead in `sales-kit/lead_pipeline.csv`.
