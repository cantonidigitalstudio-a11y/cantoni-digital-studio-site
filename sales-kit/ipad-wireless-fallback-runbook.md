...
After that trust step, rerun the WDA launch before using the iPad for future mobile-only profile/account automation. This is no longer required to accept the current Instagram official profile/link boundary. Current automated QA may still report `metadata-proof-load-error`, which is metadata proof only and not standalone public proof.
